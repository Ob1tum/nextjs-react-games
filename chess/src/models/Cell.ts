...
export class Cell {
  ...
  setHistoryMove(startCell: Cell, endCell: Cell, eatFigure: Figure | null) {
    ...
    const step = `${column[startX]}${Math.abs(Number(row[startY]) - 8)} - ${column[endX]}${Math.abs(
      Number(row[endY]) - 8,
    )}`;
    const check = !!(
      this.board.whiteKing?.underAttackKing || this.board.blackKing?.underAttackKing
    );
    this.board.historyMove.push({
      figureData: figure,
      moveData: step,
      eatFigureData: eatFigure?.logo,
      check,
    });
  }
  ...
  moveFigure(target: Cell) {
    ...
    let eatFigure = null;
    const { figure } = this; // фигура внутри ячейки
    if (figure && figure?.canMove(target)) {
      ...
      const opponentKing =
        figure.color === Colors.WHITE ? this.board.blackKing : this.board.whiteKing;
